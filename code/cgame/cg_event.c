...
/**************************************************************************/
		...
		{
			for (i = 0; i < 3; ++i)
				oldStep[i] = fabs(cg.stepChange[i] * (STEP_TIME - delta) / STEP_TIME);
		}
		else
		{
			oldStep[0] = oldStep[1] = oldStep[2] = 0;
		}
		...
		{
			oldStep[i] += step;
			if (oldStep[i] > MAX_STEP_CHANGE)
		 		oldStep[i] = MAX_STEP_CHANGE;

			cg.stepChange[i] = -oldStep[i] * Gravity[i];
		}
		cg.stepTime = cg.time;
		break;
/**********************************************************************/
