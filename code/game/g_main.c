...
=============
*/
/************* Sandro Launchpad/Teleport Stuff ********** 3/10/08 ***********/
void G_RunThink (gentity_t *ent) {
	float	thinktime, lastthinktime;

	thinktime = ent->nextthink;
	lastthinktime = ent->lastthinktime;

	if (lastthinktime > 0 && lastthinktime <= level.time) {
		ent->lastthinktime = 0;
		if (!ent->lastthink) {
			G_Error("NULL ent->lastthink");
		}
